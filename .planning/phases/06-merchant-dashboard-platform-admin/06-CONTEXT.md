...
### Support thread attachment format (resolved during UI-SPEC review)
- **D-22:** Thread attachments accept images AND PDF, not images-only. Narrows D-10's "file/image attachments" to a concrete allowlist: the existing R2/Sharp pipeline handles images as today (resize/derive), and PDFs get a small new non-image upload path through R2 that skips the Sharp derive step entirely (store as-is, no thumbnail generation). Rationale: some Mobile Money apps export a PDF receipt rather than a screenshot; the payment-claim submission flow (SUB-03, R-3) specifically benefits from this.
