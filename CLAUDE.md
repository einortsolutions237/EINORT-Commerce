...
- `resend` 6.22.0 - transactional email SDK. Declared as an optional dependency in `src/env.ts` (`RESEND_API_KEY`, `RESEND_FROM_EMAIL` both `.optional()`), with two production send-call consumers: `src/server/claims/notify.ts` (the customer payment-claim nudge to the merchant, since Phase 3) and `src/server/support/notify.ts` (the merchant↔platform support-thread nudge, since Phase 6). Both env vars stay optional by design — a project deployed without email still takes payment claims and still delivers support messages — and both consumers share the same degradation posture: a missing key produces one named `console.warn`/`console.error` and returns, never throws, because the in-app badge (claims queue count, support unread count) is the reliable channel and email is only the nudge.
